...
> - During gRPC shutdown, `GracefulStop` waits for active handlers but does not cancel a stream handler context or interrupt an in-progress `Recv`. Stream handlers that must finish within the lifecycle stop deadline need a bounded or application-owned cancellation workflow; otherwise the lifecycle force-stops the server at that deadline and clients observe a transport error.
