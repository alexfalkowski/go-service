...
//nolint:dupl
func TestSendCloseStream(t *testing.T) {
	Convey("Given I have a gRPC server", t, func() {
		lc := fxtest.NewLifecycle(t)
		logger := test.NewLogger(lc)
		cfg := test.NewInsecureTransportConfig()
		...
		s := &test.Server{
			Lifecycle: lc, Logger: logger, Tracer: tc, Transport: cfg, Meter: m,
			Stream: []grpc.StreamServerInterceptor{closingStreamInterceptor()},
		}
		s.Register()
...
//nolint:dupl
func TestSendErrorStream(t *testing.T) {
	Convey("Given I have a gRPC server", t, func() {
		lc := fxtest.NewLifecycle(t)
		logger := test.NewLogger(lc)
		cfg := test.NewInsecureTransportConfig()
		...
		s := &test.Server{
			Lifecycle: lc, Logger: logger, Tracer: tc, Transport: cfg, Meter: m,
			Stream: []grpc.StreamServerInterceptor{errorStreamInterceptor()},
		}
		s.Register()
...
func closingStreamInterceptor() grpc.StreamServerInterceptor {
