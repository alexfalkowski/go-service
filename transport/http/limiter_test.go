...
func TestServerLimiter(t *testing.T) {
	for _, f := range []string{"user-agent", "ip"} {
		t.Run(f, func(t *testing.T) {
			world := test.NewStartedHTTPWorld(t, func(world *test.World) {
				world.HandleHello()
			}, test.WithWorldTelemetry("otlp"), test.WithWorldServerLimiter(test.NewLimiterConfig(f, "1s", 0)))
...
func TestClientLimiter(t *testing.T) {
	for _, f := range []string{"user-agent", "ip"} {
		t.Run(f, func(t *testing.T) {
			world := test.NewStartedHTTPWorld(t, func(world *test.World) {
				world.HandleHello()
			}, test.WithWorldTelemetry("otlp"), test.WithWorldClientLimiter(test.NewLimiterConfig(f, "1s", 0)))
...
func TestClientClosedLimiter(t *testing.T) {
	world := test.NewStartedHTTPWorld(t, func(world *test.World) {
		world.HandleHello()
	}, test.WithWorldTelemetry("otlp"), test.WithWorldClientLimiter(test.NewLimiterConfig("user-agent", "1s", 100)))
