...
//
// It wires CloudEvents receive handlers onto a router and optionally wraps them with webhook verification
...
// NewReceiver constructs a Receiver that registers handlers on router.
//
// Registered handlers are wrapped with webhook verification middleware created from hook (if configured).
func NewReceiver(router *http.Router, hook *hooks.Webhook) *Receiver {
	return &Receiver{router: router, hook: hook}
}
