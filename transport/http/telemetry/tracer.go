...
func NewTracer(params TracerParams) (Tracer, error) {
	return telemetry.NewTracer(telemetry.TracerParams{Lifecycle: params.Lifecycle, Name: "http", Config: params.Config, Version: params.Version})
}
...
func NewTracerHandler(tracer Tracer, handler http.Handler) *TracerHandler {
	return &TracerHandler{tracer: tracer, Handler: handler}
}

// ServeHTTP for telemetry.
func (h *TracerHandler) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
...
// NewTracerRoundTripper for telemetry.
func NewTracerRoundTripper(tracer Tracer, hrt http.RoundTripper) *TracerRoundTripper {
	return &TracerRoundTripper{tracer: tracer, RoundTripper: hrt}
}

// TracerRoundTripper for telemetry.
type TracerRoundTripper struct {
	tracer Tracer
	http.RoundTripper
}

// RoundTrip for telemetry.
func (r *TracerRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
