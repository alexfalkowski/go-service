...
func verifyFeatureConfig(t *testing.T, cfg *config.Config) {
	...
	require.Equal(t, uint32(2), cfg.Feature.Breaker.MaxRequests)
...
func verifyTelemetryConfig(t *testing.T, cfg *config.Config) {
	...
	require.Equal(t, "http.server.request.duration", cfg.Telemetry.Metrics.Views[0].Pattern)
	require.Equal(t, []float64{0.005, 0.01, 0.05, 0.1, 0.5, 1, 5}, cfg.Telemetry.Metrics.Views[0].Boundaries)
	require.Equal(t, "rpc.*.duration", cfg.Telemetry.Metrics.Views[1].Pattern)
	require.Equal(t, []float64{0.01, 0.1, 1}, cfg.Telemetry.Metrics.Views[1].Boundaries)
