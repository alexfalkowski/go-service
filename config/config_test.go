...
func verifyConfig(config *config.Config) {
	...
	So(config.Cache.Options["url"], ShouldEqual, "../internal/test/secrets/redis")
	...
	So(config.Hooks.Secret, ShouldEqual, "../internal/test/secrets/hooks")
	So(len(config.SQL.PG.Masters), ShouldEqual, 1)
	So(config.SQL.PG.Masters[0].URL, ShouldEqual, "../internal/test/secrets/pg")
	So(len(config.SQL.PG.Slaves), ShouldEqual, 1)
	So(config.SQL.PG.Slaves[0].URL, ShouldEqual, "../internal/test/secrets/pg")
