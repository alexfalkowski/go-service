...
func verifyConfig(cfg *config.Config) {
	...
	So(string(cfg.Crypto.AES.Key), ShouldNotBeBlank)
	So(ed25519.IsEnabled(cfg.Crypto.Ed25519), ShouldBeTrue)
	So(string(cfg.Crypto.Ed25519.Public), ShouldNotBeBlank)
	...
	So(string(cfg.Hooks.Secret), ShouldEqual, "../test/secrets/hooks")
	...
	So(string(cfg.SQL.PG.Masters[0].URL), ShouldEqual, "../test/secrets/pg")
	So(len(cfg.SQL.PG.Slaves), ShouldEqual, 1)
	So(string(cfg.SQL.PG.Slaves[0].URL), ShouldEqual, "../test/secrets/pg")
	...
	So(cfg.Transport.HTTP.Mux, ShouldEqual, string(hm.Gateway))
