...
`secret` is a source string. The resolved value must be accepted by the
Standard Webhooks library, such as a secret generated by `hooks.Generator`
with or without the `whsec_` prefix. Empty resolved secrets fail startup.
