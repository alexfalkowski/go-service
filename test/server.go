...
func NewHTTPServer(lc fx.Lifecycle, logger *zap.Logger, cfg *telemetry.Config, tcfg *transport.Config) *shttp.Server {
	...
	server := shttp.NewServer(shttp.ServerParams{
		...
		Tracer: tracer, Metrics: htel.NewServerMetrics(lc, Version),
	})
...
func NewGRPCServer(
	lc fx.Lifecycle, logger *zap.Logger, cfg *telemetry.Config, tcfg *transport.Config,
	verifyAuth bool, unary []grpc.UnaryServerInterceptor, stream []grpc.StreamServerInterceptor,
) *tgrpc.Server {
	tracer, _ := gtel.NewTracer(gtel.TracerParams{Lifecycle: lc, Config: cfg, Version: Version})

	server := tgrpc.NewServer(tgrpc.ServerParams{
		Shutdowner: NewShutdowner(), Config: &tcfg.GRPC, Logger: logger,
		Tracer: tracer, Metrics: gtel.NewServerMetrics(lc, Version),
