...
func TestHealth(t *testing.T) {
	...
	for _, check := range checks {
		Convey("Given I register the health handler", t, func() {
			...
			o := observer(lc, "https://httpstat.us/200", test.NewHTTPClient(lc, logger, test.NewTelemetryConfig(), cfg), logger).Observe("http")
			...
			Convey(fmt.Sprintf("When I query %s", check), func() {
				client := test.NewHTTPClient(lc, logger, test.NewTelemetryConfig(), cfg)
...
func TestReadinessNoop(t *testing.T) {
	Convey("Given I register the health handler", t, func() {
		...
		logger := test.NewLogger(lc)
		cfg := test.NewTransportConfig()
		server := observer(lc, "https://httpstat.us/500", test.NewHTTPClient(lc, logger, test.NewTelemetryConfig(), cfg), logger)
...
func TestInvalidHealth(t *testing.T) {
	Convey("Given I register the health handler", t, func() {
		...
		logger := test.NewLogger(lc)
		cfg := test.NewTransportConfig()
		o := observer(lc, "https://httpstat.us/500", test.NewHTTPClient(lc, logger, test.NewTelemetryConfig(), cfg), logger).Observe("http")
		hs := test.NewHTTPServer(lc, logger, test.NewTelemetryConfig(), cfg)
		gs := test.NewGRPCServer(lc, logger, test.NewTelemetryConfig(), cfg, false, nil, nil)
